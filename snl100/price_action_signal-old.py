...
def load_ohlcv_frames() -> dict:
    ...
    for path in glob.glob(os.path.join(DATA_DIR, "ohlcv_*.csv")):
        ...
        # انتخاب ستون حجم: VolumeTo در اولویت
    ...
    for i in range(VOL_WINDOW, len(df)):
        ...
        if vol_ok and breakout_ok and hh_hl:
            ...
            signals.append({
                ...
                "profit_abs": (target - close) * capital_used,
                "profit_pct": (target - close) / close,
            ...
            signals.append({
                ...
                "position_size": capital_used,
                "capital_used": capital_used,
                "profit_abs": (close - target) * capital_used,
                "profit_pct": (close - target) / close,
