...
def orderbook_entry_logic(market, metrics, multiplier=1.5):
    ...
    stop = round(mid * (1 - bid_range * multiplier), 4)
    target = round(mid * (1 + ask_range * multiplier), 4)
...
def scan_all_symbols_v2b():
    ...
    with open("output/filter_details_v2b.json", "w", encoding="utf-8") as f:
        json.dump(details, f, ensure_ascii=False, indent=2)

    for d in details:
        market = d["market"]
        reason = d.get("reason")
        print(f"🔍 [v2b] Processing {market} → reason={reason}")
        if not d.get("ok"):
            continue
        ...
        if reason == "trend_ok_with_candles":
            ...
            if df is None or df.empty:
                ...
                entry = orderbook_entry_logic(market, d.get("metrics", {}), multiplier=1.5)
    ...
    print("✅ [v2b] اسکن کامل شد و داشبورد ساخته شد: output/dashboard.html")

if __name__ == "__main__":
    scan_all_symbols_v2b()
